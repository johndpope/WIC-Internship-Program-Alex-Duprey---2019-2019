...
def premium_analysis_df_OLS(alpha_ticker, peer_ticker_list, calib_data, analyst_upside, analyst_downside, analyst_pt_wic, as_of_dt, price_tgt_dt, metrics, metric2weight, api_host, adjustments_df_now, adjustments_df_ptd, premium_as_percent = None):
    alpha_historical_mult_df = calib_data['alpha_historical_mult_df'] # ['CID','Date','Ticker','PX','P/EPS','EV/EBITDA','EV/Sales','DVD yield','FCF yield']
    ...
    if premium_as_percent is None:
        ...
        df['Alpha Downside (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float) + df['Premium Down ($)'].astype(float))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
        df['Alpha PT WIC (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float) + df['Premium PT WIC ($)'].astype(float))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
        df['Alpha Upside (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float) + df['Premium Up ($)'].astype(float))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
    else:
        df['Premium Up (%)'] = (((df['Alpha Upside (analyst)'].astype(float) - df['Alpha Unaffected PX @ Price Target Date'].astype(float))/df['Alpha Unaffected PX @ Price Target Date'].astype(float))*100.0).fillna(0)
        df['Premium PT WIC (%)'] = (((df['Alpha PT WIC (analyst)'].astype(float) - df['Alpha Unaffected PX @ Price Target Date'].astype(float))/df['Alpha Unaffected PX @ Price Target Date'].astype(float))*100.0).fillna(0)
        df['Premium Down (%)'] = (((df['Alpha Downside (analyst)'].astype(float) - df['Alpha Unaffected PX @ Price Target Date'].astype(float))/df['Alpha Unaffected PX @ Price Target Date'].astype(float))*100.0).fillna(0)
        
        df['Alpha Downside (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float)*(1+(df['Premium Down (%)'].astype(float))/100.0))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
        df['Alpha PT WIC (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float)*(1+(df['Premium PT WIC (%)'].astype(float))/100.0))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
        df['Alpha Upside (Adj,weighted)'] = (df['Alpha Unaffected PX @ Now'].astype(float)*(1+(df['Premium Up (%)'].astype(float))/100.0))*df['Metric'].apply(lambda m: metric2weight[m]).astype(float) 
    
    return df

def premium_analysis_df(alpha_ticker, peers, as_of_dt, last_price_target_dt, analyst_upside, analyst_downside, analyst_pt_wic, metrics, metric2weight, metric2stat_rel, api_host, adjustments_df_now = None, adjustments_df_ptd = None, premium_as_percent = None):
    
    if adjustments_df_now is None:
        metric2implied_now = metric2implied_px(alpha_ticker,peers,as_of_dt,metrics,api_host,metric2stat_rel, adjustments_df = None, fperiod='1BF')
    else:
        metric2implied_now = metric2implied_px(alpha_ticker,peers,as_of_dt,metrics,api_host,metric2stat_rel, adjustments_df = adjustments_df_now, fperiod='1BF')
        
    if adjustments_df_ptd is None:
        metric2implied_at_price_tgt_date = metric2implied_px(alpha_ticker,peers,last_price_target_dt,metrics,api_host,metric2stat_rel,adjustments_df = None, fperiod='1BF')
    else:
        metric2implied_at_price_tgt_date = metric2implied_px(alpha_ticker,peers,last_price_target_dt,metrics,api_host,metric2stat_rel,adjustments_df = adjustments_df_ptd, fperiod='1BF')
    ...
    if premium_as_percent is None:
        ...
        df['Alpha PT WIC (Adj,weighted)'] = df['Alpha PT WIC (Adj)'].astype(float)*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
        df['Alpha Upside (Adj,weighted)'] = df['Alpha Upside (Adj)'].astype(float)*df['Metric'].apply(lambda m: metric2weight[m]).astype(float)
    else:
        df['Premium Up (%)'] = ((df['Alpha Upside (analyst)'] - df['Alpha Unaffected PX @ Price Target Date'])/df['Alpha Unaffected PX @ Price Target Date'])*100.0
        df['Premium PT WIC (%)'] = ((df['Alpha PT WIC (analyst)'] - df['Alpha Unaffected PX @ Price Target Date'])/df['Alpha Unaffected PX @ Price Target Date'])*100.0
        df['Premium Down (%)'] = ((df['Alpha Downside (analyst)'] - df['Alpha Unaffected PX @ Price Target Date'])/df['Alpha Unaffected PX @ Price Target Date'])*100.0
