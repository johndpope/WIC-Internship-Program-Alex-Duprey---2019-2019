...
class df_slicer:
    def __init__(self, as_of_date=None):
        #today = datetime.datetime.today() if as_of_date is None else as_of_date
        #if as_of_date is None: as_of_date = today - datetime.timedelta(days=1)
        #self.today = today

        self.as_of_date = as_of_date if not pd.isnull(as_of_date) else datetime.datetime.today()
        self.holidays = \
