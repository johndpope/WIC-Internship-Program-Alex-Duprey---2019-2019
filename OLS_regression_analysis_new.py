...
def multiple_underlying_df(ticker, start_date_yyyymmdd, end_date_yyyymmdd, api_host, fperiod='1BF'):
    '''
    Function to get the Bloomberg balance sheet for a stock at a certain date
    '''
    ...
    mkt_cap = last_elem_or_null(bbgclient.bbgclient.get_timeseries(ticker,'CUR_MKT_CAP',
                                                                   start_date_yyyymmdd,end_date_yyyymmdd, api_host=api_host))
...
def compute_implied_price_from_multiple(metric_name, multiple, mult_underlying_df):
    '''
    Caclulates the unaffected price
    '''
...
def premium_analysis_df_OLS(alpha_ticker, regression_results, analyst_upside, tgt_dt, api_host, fperiod = "1BF"):
    '''
    Calculates the downside; Does not include 3-way adjustments or balance sheet adjustments
    '''
...
def premium_analysis_df_OLS_quick(dataframes, regression_results, alpha_ticker, analyst_upside, tgt_dt, api_host, fperiod = "1BF"):
    '''
    Calculates the downside; Pre-runs collecting the metrics data to speed up the process; Used in OLS_main.py
    '''
...
def premium_analysis_df_OLS2(alpha_ticker, regression_results, analyst_upside, analyst_downside, analyst_pt_wic, tgt_dt, api_host, adjustments_df_now = None, adjustments_df_ptd = None, fperiod = "1BF"):
    '''
    Calculates downside; Inlcudes 3-way adjustments and balance sheet adjustments
    '''
...
def premium_analysis_df_OLS2_quick(dataframes, regression_results, alpha_ticker, analyst_upside, analyst_downside, analyst_pt_wic, tgt_dt,  api_host, adjustments_df_now = None, adjustments_df_ptd = None, fperiod = "1BF"):
    '''
    Calculates downside; Pre-runs collecting the metrics data to speed up the process; Used in OLS_main.py
    '''
